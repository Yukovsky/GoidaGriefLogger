...
/**
 * Контекст не-игрового изменения мира на текущем потоке (модель {@link CreateContext}).
 * Маленькие миксины в ванильную/модовую логику, которая меняет блоки прямым {@code setBlock}
 * без событий (падающие блоки, гриферство мобов), кладут сюда атрибуцию на время своей работы,
 * а универсальный перехватчик {@code LevelChunkMixin} читает её и логирует реальное изменение
 * блока с правильной позицией, {@code source_type} и системным пользователем.
 * <p>
